...
(including the new ``my_proposal_signals`` ledger), (2) produces a
*schema-valid* JSON action, and (3) reacts to the Investigator in
subsequent turns — *without* writing a permanent test that needs Ollama
to be up in CI.
...
if __package__ in {None, ""}:
    # Allow `python counterfeint/diagnostics/verify_fraudster.py` as well
    # as `python -m counterfeint.diagnostics.verify_fraudster`.
